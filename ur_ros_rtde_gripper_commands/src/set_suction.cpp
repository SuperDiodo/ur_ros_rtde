...
// ---------- PLUGIN INFO ------------------
#define PLUGIN_NAME "set_suction_command"
#define PLUGIN_CLASS_NAME SetSuction
using action_type = ur_ros_rtde_gripper_commands::action::SetSuction;
// -----------------------------------------
...
{ 
  ...
  auto suction_pin = node->get_parameter(PLUGIN_NAME ".suction_pin").as_int();

  const auto goal = goal_handle->get_goal();
  auto result = std::make_shared<ur_ros_rtde_gripper_commands::action::SetSuction::Result>();
  result->result = rtde_io->setStandardDigitalOut(suction_pin, goal->state);
...
public:
      ...
      std::shared_ptr<ur_rtde::DashboardClient> dashboard_client) override
  {
    ...
    node->declare_parameter<int>(PLUGIN_NAME ".suction_pin", 0);
