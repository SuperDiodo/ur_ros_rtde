...
// ---------- PLUGIN INFO ------------------
#define PLUGIN_NAME "move_until_contact_command"
#define PLUGIN_CLASS_NAME MoveUntilContact
using action_type = ur_ros_rtde_msgs::action::MoveUntilContact;
// -----------------------------------------
    ...
    std::shared_ptr<ur_rtde::DashboardClient> dashboard_client)
{
    ...
    result->result = rtde_control->moveUntilContact(goal->toolspeed, goal->direction, goal->acceleration);
