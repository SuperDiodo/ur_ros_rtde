...
class MoveUntilForce : public ur_ros_rtde_command
{
public:
  ...
  {
    server_ = std::make_unique<command_server_template<ur_ros_rtde_msgs::action::MoveUntilForce>>(
        node, "move_until_force_command", params, rtde_control, rtde_io, rtde_receive, dashboard_client);
  };

private:
  std::unique_ptr<command_server_template<ur_ros_rtde_msgs::action::MoveUntilForce>> server_;
};

PLUGINLIB_EXPORT_CLASS(MoveUntilForce, ur_ros_rtde_command)