...
class SetPayload : public ur_ros_rtde_command
{
public:
    ...
    {
        server_ = std::make_unique<command_server_template<ur_ros_rtde_msgs::action::SetPayload>>(
            node, "set_payload_command", params, rtde_control, rtde_io, rtde_receive, dashboard_client);
    };

private:
    std::unique_ptr<command_server_template<ur_ros_rtde_msgs::action::SetPayload>> server_;
};

PLUGINLIB_EXPORT_CLASS(SetPayload, ur_ros_rtde_command)