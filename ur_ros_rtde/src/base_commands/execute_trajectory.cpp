    ...
    std::shared_ptr<ur_rtde::DashboardClient> dashboard_client)
{
  ...
  bool parametrization_success = trajectory_time_planning(msg_to_Array6dVector(goal->trajectory), goal->acceleration, goal->speed, servo_j_timestep, times, velocities, iterations, samples);
...
#define COMPUTE_WAYPOINT_TO_CHECK(x) x - lookahead_delay
  ...
  while (true)
  {
    ...
    delayed_waypoint_idx = COMPUTE_WAYPOINT_TO_CHECK(sample_idx);
    ...
    {
      t_start = rtde_control->initPeriod();
      result->result = rtde_control->servoJ(Array6dToVector(p.p), goal->speed, goal->acceleration, servo_j_timestep, servo_j_lookahead_time, servo_j_gain);
      if (!result->result)
      {
    ...
    {
      result->result = false;
      RCLCPP_INFO(node->get_logger(), "Aborting because the deviation from trajectory is greater than %f rad: %f rad", max_allowed_deviation, deviation_old_waypoint);
    ...
    auto distance_to_goal = (actual_conf - end_conf).norm();
    if (sample_idx == num_trajectory_samples - 1 && distance_to_goal < 5.0E-3)
    {
      /* move to the last configuration, just to be sure */
      rtde_control->servoStop(std::max(goal->deceleration, 1.0));
      rtde_control->moveJ(Array6dToVector(end_sample.p), 0.2, 0.2);
      rtde_control->stopJ();
      result->result = true;
      RCLCPP_INFO(node->get_logger(), "Reached a distance of %f rad from goal, terminating trajectory execution", distance_to_goal);
      break;
    }
    ...
    for(auto jj = 0; jj < prev_prev_p.v.size(); jj++){
      if(std::signbit(prev_prev_p.v[jj]) != std::signbit(prev_p.v[jj])){
        is_change_direction = true;
        //std::cout << sample_idx-1 << " occured in direction change, " << prev_prev_p.v[jj] << " ~ " << prev_p.v[jj] << std::endl;
        break;
      }
    }

    if (!is_change_direction && dist_prev_prev < deviation_old_waypoint && dist_prev_prev > 1.0E-2)
    {
      //std::cout << sample_idx << " ~ " << delayed_waypoint_idx << std::endl;
      //for (auto jj : prev_prev_p.p) std::cout << jj << " ";
      //std::cout << std::endl;
      //for (auto jj : q) std::cout << jj << " ";
      //std::cout << std::endl;
      //for (auto jj : prev_p.p) std::cout << jj << " ";
      //std::cout << std::endl << std::endl;
      //RCLCPP_INFO(node->get_logger(), "Sample %d robot is too far from the next goal (%f ~ %f rad), sending again this sample", sample_idx, dist_prev_prev, deviation_old_waypoint);
      sample_idx--;
