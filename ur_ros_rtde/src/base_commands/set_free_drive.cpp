...
// ---------- PLUGIN INFO ------------------
#define PLUGIN_NAME "set_freedrive_command"
#define PLUGIN_CLASS_NAME SetFreedrive
using action_type = ur_ros_rtde_msgs::action::SetFreedrive;
// -----------------------------------------
...
{ 
  ...
  if (goal->activated)
  {
    result->result = rtde_control->freedriveMode(goal->free_axes, goal->feature);
  }
  else
    result->result = rtde_control->endFreedriveMode();
