...
class SetSpeedSlider : public ur_ros_rtde_command
{
public:
    ...
    {
        server_ = std::make_unique<command_server_template<ur_ros_rtde_msgs::action::SetSpeedSlider>>(
            node, "set_speed_slider_command", params, rtde_control, rtde_io, rtde_receive, dashboard_client);
    };

private:
    std::unique_ptr<command_server_template<ur_ros_rtde_msgs::action::SetSpeedSlider>> server_;
};

PLUGINLIB_EXPORT_CLASS(SetSpeedSlider, ur_ros_rtde_command)