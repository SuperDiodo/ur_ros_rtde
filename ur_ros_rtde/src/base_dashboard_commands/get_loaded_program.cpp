...
// ---------- PLUGIN INFO ------------------
#define PLUGIN_NAME "get_loaded_program_dashboard_command"
#define PLUGIN_CLASS_NAME GetLoadedProgram
using action_type = ur_ros_rtde_msgs::action::GetLoadedProgram;
// -----------------------------------------
    ...
    std::shared_ptr<ur_rtde::DashboardClient> dashboard_client)
{
  ...
  try
  {
    result->result = dashboard_client->getLoadedProgram();
  }
  catch (...)
  {
    succeded = false;
